...
##TV control
enum tvState {init, firstWatch, firstRewind, rewinding, selecting, controlling, waitingForInput, watching, watchingStart, finishedFirstWatching, waiting, finishedWatching, doorTalking}
...
func _show_puzzle_completed_dialogue():
	...
	dialogueLabel.append_text("A gang tattoo...?\n")
	dialogueLabel.append_text("Hmm...\n")
	...
	await get_tree().create_timer(0.5).timeout
	_tween = create_tween() # reset tween object for next animation (needed to separate sound fxs)
	_tween.tween_property(dialogueLabel, "visible_characters", 45, 1.5).from(14)
	_load_sound_fx("talk")
	currentTvState = tvState.doorTalking
	await _tween.finished
	_tween = create_tween() # reset tween object for next animation (needed to separate sound fxs)
	_tween.tween_property(dialogueLabel, "visible_characters", 63, 1.5).from(45)
	await _tween.finished
	_tween = create_tween() # reset tween object for next animation (needed to separate sound fxs)
	_tween.tween_property(dialogueLabel, "visible_characters", 70, 1.5).from(63)
	await _tween.finished
	_tween = create_tween() # reset tween object for next animation (needed to separate sound fxs)
	_tween.tween_property(dialogueLabel, "visible_characters", 89, 1.5).from(70)
	n_visible_chars = 0 # reset the visible chars for next talking dialogue	
	DOOR_CAMERA.priority = 0
	TV_CAMERA.priority = 2
	dialoguePointer.show()
